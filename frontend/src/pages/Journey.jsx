...
import { JOURNEY, EXPERIENCE } from "../data/content";
  ...
  return (
    <div className="mx-auto max-w-[1440px] px-5 sm:px-8 py-16 sm:py-24">
      <SectionHead
        num="05 /"
        title="JOURNEY.LOG"
        sub="A career rendered as commit history. Progression, not pretense."
      />
      ...
      <div className="grid lg:grid-cols-12 gap-14">
        <div className="lg:col-span-5">
          <div className="panel p-6 sm:p-8">
            ...
            <div className="space-y-0">
              {JOURNEY.map((j, i) => (
                <Reveal key={j.hash} delay={i * 0.08} className="relative pl-10 pb-10 last:pb-0">
                  {i < JOURNEY.length - 1 && (
                  ...
                  <span className="absolute left-0 top-1 grid place-items-center w-7 h-7 border border-line bg-card">
                    <GitCommitHorizontal size={13} className={i === JOURNEY.length - 1 ? "text-grn" : "text-violet"} />
                  </span>
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                    <span className="font-mono text-[11px] text-violet">{j.hash}</span>
                    <span className="font-mono text-[9px] tracking-[0.2em] text-ink3">{j.year} · {j.branch}</span>
                  </div>
                  <h3 className={`mt-1.5 font-mono text-sm font-bold tracking-wide ${i === JOURNEY.length - 1 ? "text-grn" : "text-ink"}`}>
                    {j.title}
                  </h3>
        ...
        <div className="lg:col-span-7">
          ...
          <div className="space-y-5">
            {EXPERIENCE.map((e, i) => (
              <Reveal key={e.role + e.org} delay={i * 0.06} className="panel panel-hover p-6 sm:p-7">
                <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                  ...
                  <div className="text-right">
                    <span className="font-mono text-[10px] tracking-[0.15em] text-violet block">{e.period}</span>
                    <span className="font-mono text-[9px] tracking-[0.2em] text-ink3">{e.tag}</span>
                  </div>
                </div>
