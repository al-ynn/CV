  ...
  return (
    <div className="mx-auto max-w-[1440px] px-5 sm:px-8 py-16 sm:py-24">
      <SectionHead num="03 /" title={`SERVICES / ${String(services.length || 0).padStart(2, "0")}`}
        sub="Service systems. Each capability is labeled with its actual status — core strength, experience, or available on request." />
        ...
        {services.map((s) => {
          ...
          return (
            <Reveal key={s.id} className={`panel ${isOpen ? "border-violet" : "panel-hover"}`}>
              ...
              >
                <span className="font-mono text-[11px] tracking-[0.25em] text-violet shrink-0">SERVICE / {s.num}</span>
                <div className="flex-1 min-w-0">
                  <h2 className="font-display text-base sm:text-xl font-extrabold tracking-tight text-ink truncate">{s.title}</h2>
                ...
                <ChevronDown size={16} className={`text-ink3 shrink-0 transition-transform duration-300 ${isOpen ? "rotate-180 text-violet" : ""}`} />
              </button>
                  ...
                  >
                    <div className="px-5 sm:px-6 pb-6">
                      ...
                      <div className="mt-5 flex justify-end">
                        <Link to="/contact" data-testid={`service-inquire-${s.id}`}
