...
import { MARQUEE_ITEMS, STACK, PROCESS, JOURNEY } from "../data/content";
...
const HERO_LINES = ["I BUILD SYSTEMS", "THAT WORK BEYOND", "THE INTERFACE."];
const BARS = [
...
export default function Home() {
  const { projects, services, pricing, settings, loading } = useContent();
    ...
    <div>
      ...
      <section ref={heroRef} className="relative bg-grid overflow-hidden">
        <div className="mx-auto max-w-[1440px] px-5 sm:px-8 pt-16 sm:pt-24 pb-16 sm:pb-20 grid lg:grid-cols-12 gap-12 items-center">
          <motion.div className="lg:col-span-7" style={{ opacity: heroOpacity }}>
            ...
            >
              <span className="font-mono text-[10px] tracking-[0.3em] text-violet">FULL-STACK DEVELOPER / PHILIPPINES</span>
              ...
              {HERO_LINES.map((line, i) => (
                <span key={line} className="block overflow-hidden pb-1">
                  ...
                  >
                    {i === 2 ? (
                      <>
                        THE <span className="text-violet">INTERFACE.</span>
                      </>
                    ) : (
                      line
                    )}
            ...
            >
              I design and develop full-stack web applications, information systems, e-commerce
              platforms, APIs, databases, and digital experiences — from idea and interface to deployment.
            </motion.p>
            ...
            <motion.div
              ...
              className="mt-6 flex items-center gap-2 font-mono text-[10px] tracking-[0.25em] uppercase"
              data-testid="hero-status"
            >
              <StatusDot />
              <span className="text-grn">Available for select projects</span>
            </motion.div>
            ...
            >
              <Link
                ...
                style={{ color: "var(--bg)" }}
              >
                <SwapText label="Start a Project →" alt="Initialize Inquiry →" />
              </Link>
              <Link
                to="/work"
              ...
              >
                <SwapText label="Explore My Work" alt="Open /Project Archive" />
              </Link>
              <a
      ...
      <section className="border-y border-line">
        <div className="mx-auto max-w-[1440px] grid grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { k: "DEPLOYED SYSTEMS", v: String(projects.length || 5).padStart(2, "0"), c: "var(--violet)" },
            { k: "SERVICE MODULES", v: String(services.length || 7).padStart(2, "0"), c: "var(--cyan)" },
            { k: "EDUCATION", v: "BS IT · CLSU", c: "var(--amber)" },
          ].map((m, i) => (
            <Reveal key={m.k} delay={i * 0.06} className={`px-5 sm:px-8 py-7 border-line ${i > 0 ? "border-l" : ""} ${i > 1 ? "max-lg:border-t max-lg:border-l-0" : ""} ${i % 2 === 1 ? "max-lg:border-l" : ""}`}>
              <TechLabel className="block mb-2">{m.k}</TechLabel>
              <span className="font-mono text-xl sm:text-2xl font-bold tabular-nums" style={{ color: m.c }}>
                {m.v}
              </span>
            </Reveal>
          ))}
      ...
      <section className="mx-auto max-w-[1440px] px-5 sm:px-8 py-20 sm:py-28">
        <SectionHead
          ...
          right={
            <Link to="/work" data-testid="home-all-work" className="group font-mono text-[10px] tracking-[0.2em] uppercase text-ink2 hover:text-violet transition-colors whitespace-nowrap">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px bg-line border border-line">
            {WHAT_I_BUILD.map((w, i) => (
              <Reveal key={w.num} delay={i * 0.07} className="bg-card p-7 group hover:bg-canvas2/60 transition-colors">
                ...
                <h3 className="mt-4 font-display text-lg font-bold tracking-tight text-ink group-hover:text-violet transition-colors">
                  {w.title}
                </h3>
      ...
      <section className="mx-auto max-w-[1440px] px-5 sm:px-8 py-20 sm:py-28">
        <SectionHead
          ...
          sub="Seven service systems. Each one a stack of specialized capabilities."
        />
      ...
      <section className="border-y border-line bg-canvas2/40">
        <div className="mx-auto max-w-[1440px] px-5 sm:px-8 py-20 sm:py-28">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {STACK.map((cat, i) => (
              <Reveal key={cat.category} delay={i * 0.05} className="panel p-6">
                <TechLabel className="block mb-4 text-violet">{cat.category}</TechLabel>
          ...
          {PROCESS.map((step, i) => (
            <Reveal key={step.num} delay={i * 0.06} className="bg-card p-6 relative group hover:bg-canvas2/60 transition-colors">
              ...
              <ul className="space-y-1.5">
                {step.items.map((it) => (
                  <li key={it} className="font-mono text-[10px] tracking-[0.08em] text-ink3 uppercase">
                    · {it}
                  </li>
                ))}
              </ul>
      ...
      <section className="border-y border-line bg-canvas2/40">
        <div className="mx-auto max-w-[1440px] px-5 sm:px-8 py-20 sm:py-28 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="panel p-6 font-mono text-xs space-y-3">
            ...
            {JOURNEY.slice(-3).map((j, i) => (
              <Reveal key={j.hash} delay={i * 0.08} className="flex gap-4">
                <span className="text-violet shrink-0">{j.hash}</span>
      ...
      <section className="mx-auto max-w-[1440px] px-5 sm:px-8 py-20 sm:py-28">
        <SectionHead
          ...
          right={
            <Link to="/pricing" data-testid="home-pricing-link" className="group font-mono text-[10px] tracking-[0.2em] uppercase text-ink2 hover:text-violet transition-colors whitespace-nowrap">
          ...
          <Reveal>
            ...
            <div className="mt-10 flex flex-wrap justify-center gap-4">
              ...
              <Link
                ...
                data-testid="home-estimator-cta"
