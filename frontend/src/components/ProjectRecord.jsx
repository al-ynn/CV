  ...
  return (
    ...
    >
      <Link
        ...
        className={`group block panel panel-hover relative overflow-hidden ${large ? "p-7 sm:p-10" : "p-6"}`}
      >
        ...
        <div className="flex items-start justify-between gap-4 mb-5">
          <span className="font-mono text-[11px] tracking-[0.3em] text-violet">
            PROJECT / {p.num}
          </span>
          <span
        ...
        <h3 className={`font-display font-extrabold tracking-tight text-ink leading-none ${large ? "text-3xl sm:text-4xl" : "text-2xl"}`}>
          {p.title}
        </h3>
        <p className="mt-1.5 font-mono text-[11px] tracking-[0.2em] uppercase text-ink3">{p.subtitle}</p>

        <p className={`mt-4 text-sm text-ink2 leading-relaxed ${large ? "" : "line-clamp-2"}`}>{p.description}</p>
